package o;

import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.view.MotionEvent;
import java.io.PrintWriter;
import java.io.StringWriter;
import java.io.Writer;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Calendar;
import java.util.Collections;
import java.util.Date;
import java.util.LinkedList;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public abstract class jv
  extends jh
{
  private static volatile kb arc = null;
  private static Method asK;
  private static boolean asM;
  private static Object asP = new Object();
  private static long startTime = 0L;
  protected boolean asJ = false;
  private String asL;
  private boolean asN = false;
  private boolean asO = false;
  
  static
  {
    asM = false;
  }
  
  protected jv(Context paramContext, String paramString)
  {
    super(paramContext);
    asL = paramString;
    asJ = false;
  }
  
  protected jv(Context paramContext, String paramString, boolean paramBoolean)
  {
    super(paramContext);
    asL = paramString;
    asJ = paramBoolean;
  }
  
  protected static void ʼ(List<Callable<Void>> paramList)
  {
    if (arc == null) {
      return;
    }
    Object localObject = arcatb;
    if (localObject == null) {
      return;
    }
    if (!paramList.isEmpty()) {
      try
      {
        mn localmn = ms.awM;
        ((ExecutorService)localObject).invokeAll(paramList, ((Long)v.כֿ().ˊ(localmn)).longValue(), TimeUnit.MILLISECONDS);
        return;
      }
      catch (InterruptedException paramList)
      {
        localObject = new StringWriter();
        paramList.printStackTrace(new PrintWriter((Writer)localObject));
        ((StringWriter)localObject).toString();
      }
    }
  }
  
  private static List<Long> ˊ(kb paramkb, MotionEvent paramMotionEvent, DisplayMetrics paramDisplayMetrics)
  {
    paramkb = paramkb.ﾞ("7scYqzyHRPBaZWFKJ3pOWHbR6Dbo5le4dynIUtP3L7pYFHAqNzdBRQatrNTDhiks", "vVlHDsOifDC8W64bgexaMgYAPimhsdV/psSFMo/Evqg=");
    asK = paramkb;
    if ((paramkb == null) || (paramMotionEvent == null)) {
      throw new ka();
    }
    try
    {
      paramkb = (ArrayList)asK.invoke(null, new Object[] { paramMotionEvent, paramDisplayMetrics });
      return paramkb;
    }
    catch (IllegalAccessException paramkb)
    {
      throw new ka(paramkb);
    }
    catch (InvocationTargetException paramkb)
    {
      throw new ka(paramkb);
    }
  }
  
  protected static void ˋ(Context paramContext, boolean paramBoolean)
  {
    try
    {
      if (!asM)
      {
        startTime = Calendar.getInstance().getTime().getTime() / 1000L;
        arc = ˎ(paramContext, paramBoolean);
        asM = true;
      }
      return;
    }
    finally
    {
      paramContext = finally;
      throw paramContext;
    }
  }
  
  private static kb ˎ(Context paramContext, boolean paramBoolean)
  {
    if (arc == null) {
      synchronized (asP)
      {
        if (arc == null)
        {
          paramContext = kb.ˊ(paramContext, "QddrLEAjpgWETBLVY7oqqhC9aEVon4NZsbEEwuFsz2A=", "", paramBoolean);
          List localList = Collections.singletonList(Context.class);
          paramContext.ˊ("FpxkL8T4+4KvQMD9i+2pSkm0iSIrm7YPofeiNCCHrFTofQNLiBOBNdZP5Pz5i3jB", "WFZF3SqWUN2v06LedHaqBGrhE85rZge34n+mPFIupKU=", localList);
          paramContext.ˊ("lLpTIaE60qRmDJilKTnB6dMslmEDCMG+aJ7xPwxeE01HtxatTPhAFeGxL2EFpKqq", "LwAyv7R7EEW6/T7p6KlsghmfaITLnkCV2ffewHyZJ4E=", localList);
          paramContext.ˊ("WDNcg/qrUvZ/arv3KlIpjTJy2PLrL5zJRy6RhtE1BwGO7yUZTah8sIwGKrLNQxBR", "k1O5CScpibVjQ/AkqNOTz2L1NxiUwSDOHHe20mOBuZE=", localList);
          paramContext.ˊ("0ghH/t1vDAhJsMBEq6AmO4wSmIDUPcR/Ca+bXIPvotbCa7WG11X2GY3bGGSFRSWI", "UxLcO2gMVJYxcPCRIA0oswQMYaa0vipadHgqkbOQas0=", localList);
          paramContext.ˊ("keK3XK0wK2UU/nLysKwyCf+kkLkTOC2vAbYqwrBu7R43EUtKstTw0Ncacr5N836s", "waGnxH9bEhira4fPRFV2xqpjD0WpWaSdKy82IuNFwdk=", localList);
          paramContext.ˊ("L05xKnfnYeHbEBu8dBVHLX06/lqdQ3wjnf1MDhbz8UyVOSERj3ew9tHuZCsFIQLQ", "alpbMH9e2rlIOT//YFdQTXyhxLWZXiidj7upE61JEUs=", localList);
          paramContext.ˊ("DlgHLXqDLqm1GFukOQGzXSwndUeIXE36A5gg71kBD49NoHWHGWFjde/o+K/rzyX6", "rvMoYpysLXzhZ6Icu/Rx+8e3b8bA+ziXoZXmih9N3OA=", localList);
          paramContext.ˊ("fBtIGCHte1o+4DEJXJKmN8amPgr7INw+5aLJWfquR6onmnM2N4yyUIpZKWlQd2MT", "UbZQKWrvtEsWMuqKUZ59vHKULlKuI6WSXmDlyXvVcPA=", localList);
          paramContext.ˊ("8XC+T5ZJmGqLgq9bGckp9QpSek/MdZkWG8J7h0S/jQT+nLtpFloYeQEp8BYxEnxS", "ybUDDh+rxXFJD95YxLPryhWUtCqqCbMTY7q0vd/SZrg=", localList);
          paramContext.ˊ("7scYqzyHRPBaZWFKJ3pOWHbR6Dbo5le4dynIUtP3L7pYFHAqNzdBRQatrNTDhiks", "vVlHDsOifDC8W64bgexaMgYAPimhsdV/psSFMo/Evqg=", Arrays.asList(new Class[] { MotionEvent.class, DisplayMetrics.class }));
          paramContext.ˊ("L5suOEkl11eDXKBlerzt5uhrXmMUgM/zg/0p0sGGN2whxBlK6x9tQhm9iwezF+oF", "LKckwzQJypLfshE2gWJ2grGramB5zEfM8v/nJLs8qn0=", Collections.emptyList());
          paramContext.ˊ("6p7iCXfSUdPVZxKIMLb6kQ6w4NNjoD9lvLRgWpY6QIdh4oP6AFLdVKBHQR56jQnq", "am+Emx+Il9MpMu9RJG55dNiUlw7VvmwKoBU1NE91gtY=", Collections.emptyList());
          paramContext.ˊ("BOJ3tjkSPl7I5QAxVTrljV+nhFJcIx2Q/kO7zmVvITuSUJHDxQAfVy/jOA0v9pYs", "zDiqPrOORPXZIrDdW7RtGAel/ckCjtoUBGAnfbt1Dbs=", Collections.emptyList());
          paramContext.ˊ("gK2idbBMZeT4po0Euq26hpNJPtYADahIoduvc+9lPfaj98i6doa/I7yVE/XdPW0n", "z9ajAOx5ZNdFgTi1Ek7alAp0ZDId6vOmAD0S3qFva+s=", Collections.emptyList());
          paramContext.ˊ("58FzSXiuhwkPJhngNzopCJPRsb4QxaL4R9w88HtiTtPngj9cSA9bk253tVUsvdvn", "FSmb+R06dYXhtkSUnpYyddV7qH5CUpQhJaF+z8pCRgU=", Collections.emptyList());
          arc = paramContext;
        }
      }
    }
    return arc;
  }
  
  protected final is.if ʲ(Context paramContext)
  {
    is.if localif = new is.if();
    if (!TextUtils.isEmpty(asL)) {
      RM = asL;
    }
    paramContext = ˎ(paramContext, asJ);
    paramContext.ค();
    ˊ(paramContext, localif);
    paramContext.ฅ();
    return localif;
  }
  
  protected void ˊ(kb paramkb, is.if paramif)
  {
    if (atb == null) {
      return;
    }
    ʼ(ˋ(paramkb, paramif));
  }
  
  protected List<Callable<Void>> ˋ(kb paramkb, is.if paramif)
  {
    int i = Integer.MIN_VALUE;
    if (atj != null) {
      i = je.ף();
    }
    ArrayList localArrayList = new ArrayList();
    localArrayList.add(new kh(paramkb, "FpxkL8T4+4KvQMD9i+2pSkm0iSIrm7YPofeiNCCHrFTofQNLiBOBNdZP5Pz5i3jB", "WFZF3SqWUN2v06LedHaqBGrhE85rZge34n+mPFIupKU=", paramif, i));
    localArrayList.add(new km(paramkb, "L5suOEkl11eDXKBlerzt5uhrXmMUgM/zg/0p0sGGN2whxBlK6x9tQhm9iwezF+oF", "LKckwzQJypLfshE2gWJ2grGramB5zEfM8v/nJLs8qn0=", paramif, startTime, i));
    localArrayList.add(new kr(paramkb, "BOJ3tjkSPl7I5QAxVTrljV+nhFJcIx2Q/kO7zmVvITuSUJHDxQAfVy/jOA0v9pYs", "zDiqPrOORPXZIrDdW7RtGAel/ckCjtoUBGAnfbt1Dbs=", paramif, i));
    localArrayList.add(new ks(paramkb, "0ghH/t1vDAhJsMBEq6AmO4wSmIDUPcR/Ca+bXIPvotbCa7WG11X2GY3bGGSFRSWI", "UxLcO2gMVJYxcPCRIA0oswQMYaa0vipadHgqkbOQas0=", paramif, i));
    localArrayList.add(new kt(paramkb, "6p7iCXfSUdPVZxKIMLb6kQ6w4NNjoD9lvLRgWpY6QIdh4oP6AFLdVKBHQR56jQnq", "am+Emx+Il9MpMu9RJG55dNiUlw7VvmwKoBU1NE91gtY=", paramif, i));
    localArrayList.add(new kg(paramkb, "WDNcg/qrUvZ/arv3KlIpjTJy2PLrL5zJRy6RhtE1BwGO7yUZTah8sIwGKrLNQxBR", "k1O5CScpibVjQ/AkqNOTz2L1NxiUwSDOHHe20mOBuZE=", paramif, i));
    localArrayList.add(new kk(paramkb, "keK3XK0wK2UU/nLysKwyCf+kkLkTOC2vAbYqwrBu7R43EUtKstTw0Ncacr5N836s", "waGnxH9bEhira4fPRFV2xqpjD0WpWaSdKy82IuNFwdk=", paramif, i));
    localArrayList.add(new kq(paramkb, "fBtIGCHte1o+4DEJXJKmN8amPgr7INw+5aLJWfquR6onmnM2N4yyUIpZKWlQd2MT", "UbZQKWrvtEsWMuqKUZ59vHKULlKuI6WSXmDlyXvVcPA=", paramif, i));
    localArrayList.add(new ke(paramkb, "8XC+T5ZJmGqLgq9bGckp9QpSek/MdZkWG8J7h0S/jQT+nLtpFloYeQEp8BYxEnxS", "ybUDDh+rxXFJD95YxLPryhWUtCqqCbMTY7q0vd/SZrg=", paramif, i));
    localArrayList.add(new kj(paramkb, "L05xKnfnYeHbEBu8dBVHLX06/lqdQ3wjnf1MDhbz8UyVOSERj3ew9tHuZCsFIQLQ", "alpbMH9e2rlIOT//YFdQTXyhxLWZXiidj7upE61JEUs=", paramif, i));
    localArrayList.add(new ki(paramkb, "DlgHLXqDLqm1GFukOQGzXSwndUeIXE36A5gg71kBD49NoHWHGWFjde/o+K/rzyX6", "rvMoYpysLXzhZ6Icu/Rx+8e3b8bA+ziXoZXmih9N3OA=", paramif, i));
    ml localml = ms.awO;
    if (((Boolean)v.כֿ().ˊ(localml)).booleanValue()) {
      localArrayList.add(new kl(paramkb, "gK2idbBMZeT4po0Euq26hpNJPtYADahIoduvc+9lPfaj98i6doa/I7yVE/XdPW0n", "z9ajAOx5ZNdFgTi1Ek7alAp0ZDId6vOmAD0S3qFva+s=", paramif, i));
    }
    localml = ms.awQ;
    if (((Boolean)v.כֿ().ˊ(localml)).booleanValue()) {
      localArrayList.add(new kp(paramkb, "58FzSXiuhwkPJhngNzopCJPRsb4QxaL4R9w88HtiTtPngj9cSA9bk253tVUsvdvn", "FSmb+R06dYXhtkSUnpYyddV7qH5CUpQhJaF+z8pCRgU=", paramif, i));
    }
    return localArrayList;
  }
  
  protected is.if ː(Context paramContext)
  {
    is.if localif = new is.if();
    if (!TextUtils.isEmpty(asL)) {
      RM = asL;
    }
    paramContext = ˎ(paramContext, asJ);
    paramContext.ค();
    try
    {
      localArrayList = ˊ(paramContext, arx, arF);
      agS = ((Long)localArrayList.get(0));
      agT = ((Long)localArrayList.get(1));
      if (((Long)localArrayList.get(2)).longValue() >= 0L) {
        agU = ((Long)localArrayList.get(2));
      }
      ahi = ((Long)localArrayList.get(3));
      ahj = ((Long)localArrayList.get(4));
    }
    catch (ka localka1)
    {
      ArrayList localArrayList;
      for (;;) {}
    }
    if (arz > 0L) {
      ahn = Long.valueOf(arz);
    }
    if (arA > 0L) {
      ahm = Long.valueOf(arA);
    }
    if (arB > 0L) {
      ahl = Long.valueOf(arB);
    }
    if (arC > 0L) {
      aho = Long.valueOf(arC);
    }
    if (arD > 0L) {
      ahq = Long.valueOf(arD);
    }
    try
    {
      int j = ary.size() - 1;
      if (j > 0)
      {
        ahr = new is.if.if[j];
        i = 0;
        while (i < j)
        {
          localArrayList = ˊ(paramContext, (MotionEvent)ary.get(i), arF);
          localObject = new is.if.if();
          agS = ((Long)localArrayList.get(0));
          agT = ((Long)localArrayList.get(1));
          ahr[i] = localObject;
          i += 1;
        }
      }
    }
    catch (ka localka2)
    {
      int i;
      Object localObject;
      for (;;) {}
    }
    ahr = null;
    localArrayList = new ArrayList();
    if (atb != null)
    {
      i = Integer.MIN_VALUE;
      if (atj != null) {
        i = je.ף();
      }
      localArrayList.add(new ko(paramContext, localif));
      localArrayList.add(new kr(paramContext, "BOJ3tjkSPl7I5QAxVTrljV+nhFJcIx2Q/kO7zmVvITuSUJHDxQAfVy/jOA0v9pYs", "zDiqPrOORPXZIrDdW7RtGAel/ckCjtoUBGAnfbt1Dbs=", localif, i));
      localArrayList.add(new km(paramContext, "L5suOEkl11eDXKBlerzt5uhrXmMUgM/zg/0p0sGGN2whxBlK6x9tQhm9iwezF+oF", "LKckwzQJypLfshE2gWJ2grGramB5zEfM8v/nJLs8qn0=", localif, startTime, i));
      localObject = ms.awP;
      if (((Boolean)v.כֿ().ˊ((mk)localObject)).booleanValue()) {
        localArrayList.add(new kl(paramContext, "gK2idbBMZeT4po0Euq26hpNJPtYADahIoduvc+9lPfaj98i6doa/I7yVE/XdPW0n", "z9ajAOx5ZNdFgTi1Ek7alAp0ZDId6vOmAD0S3qFva+s=", localif, i));
      }
      localArrayList.add(new ke(paramContext, "8XC+T5ZJmGqLgq9bGckp9QpSek/MdZkWG8J7h0S/jQT+nLtpFloYeQEp8BYxEnxS", "ybUDDh+rxXFJD95YxLPryhWUtCqqCbMTY7q0vd/SZrg=", localif, i));
      localObject = ms.awR;
      if (((Boolean)v.כֿ().ˊ((mk)localObject)).booleanValue()) {
        localArrayList.add(new kp(paramContext, "58FzSXiuhwkPJhngNzopCJPRsb4QxaL4R9w88HtiTtPngj9cSA9bk253tVUsvdvn", "FSmb+R06dYXhtkSUnpYyddV7qH5CUpQhJaF+z8pCRgU=", localif, i));
      }
    }
    ʼ(localArrayList);
    paramContext.ฅ();
    return localif;
  }
}

/* Location:
 * Qualified Name:     o.jv
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */